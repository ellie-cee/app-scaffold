...
def appointment_request_submit(request):
    ...
    if request.method == 'POST':
        ...
        print("request submit",request.POST)
        if form.is_valid():
...
def appointment_client_information(request, appointment_request_id, id_request):
    ...
    if request.method == 'POST':
        ...
        if not payload.get("phone").startswith("+1"):
            payload["phone"]= f"+1{payload.get('phone')}"
            
        appointment_form = AppointmentForm(payload)
        
        client_data_form = ClientDataForm(payload)
        
        print(appointment_form.is_valid(),client_data_form.is_valid())
        if client_data_form.is_valid() and appointment_form.is_valid():
            ...
            request.session['want_reminder'] = payload.get("want_reminder")
